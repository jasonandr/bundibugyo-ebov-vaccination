...
def generate_network(N=10000, household_mean=5.0, community_mean=5.0, community_variance=25.0):
    """
    ...
    hh_counter = 0
    while idx < N:
        # Household/caregiving size: shifted Poisson with configurable mean.
...
def calibrate_tau(G, target_R0, gamma, num_trials=50):
    """
    Empirically Simulation-Based Calibration.
    Finds the baseline transmission probability (tau) that produces the target R0 on a highly clustered network.
    Because theoretical approximation breaks down when clustering > 0, 
    we must empirically sweep to find the tau that offsets the "wasted" contacts within saturated cliques.
    """
    taus = np.linspace(0.01, 0.4, 20)
    best_tau = 0.01
    min_diff = float('inf')
    ...
    for tau in taus:
        ...
        for _ in range(num_trials):
            index_node = np.random.choice(G.nodes())
            inf_time = np.random.exponential(1.0 / gamma)
            
            secondary = 0
            for neighbor in G.neighbors(index_node):
                if np.random.rand() < (1.0 - np.exp(-tau * inf_time)):
                    secondary += 1
            r0_estimates.append(secondary)
            
        empirical_r0 = np.mean(r0_estimates)
        diff = abs(empirical_r0 - target_R0)
        if diff < min_diff:
            min_diff = diff
            best_tau = tau
            
    # Fine-tuning
    taus_fine = np.linspace(max(0.001, best_tau - 0.02), best_tau + 0.02, 15)
...
def simulate_ring_vaccination(G, rt_array=None, baseline_tau=0.25, 
                              ...
                              sigmoidal_k=None, sigmoidal_d0=None,
                              uptake_r2_drop=0.75, tracing_delay_r2_add=2.0, compete_queue=False, allow_pep=True, community_vax_coverage=0.0, community_vax_trigger=0, community_vax_delay=-1.0, community_vax_rollout_days=0.0, seed=None, engine='python', return_mechanism=False,
                              infection_efficacy_multiplier=1.0, immediate_mortality_protection=False):
    """
    ...
    if engine == 'cpp':
        import ebola_stochastic_ring_old_cpp
        N = G.number_of_nodes()
        if hasattr(G, 'cached_adj'):
            adj = G.cached_adj
        else:
            adj = [[int(x) for x in G.neighbors(i)] for i in range(N)]
            G.cached_adj = adj
        ...
        if return_mechanism:
            res = ebola_stochastic_ring_old_cpp.simulate_mechanism_cpp(
                int(N), adj, [float(x) for x in rt_array] if rt_array else [], float(baseline_tau),
            ...
            res = ebola_stochastic_ring_old_cpp.simulate_ring_vaccination_cpp(
                int(N), adj, [float(x) for x in rt_array] if rt_array else [], float(baseline_tau),
                float(incubation_period), float(infectious_period),
                int(ring_radius), float(efficacy), float(immune_delay), float(uptake),
                [float(x) for x in rr_array] if rr_array else [], float(rr_scalar),
    ...
    daily_deaths = np.zeros(max_sim_time + 1)
    ...
    while queue:
        ...
        if event_type == 'EXPOSURE':
            ...
            prob_accept = 1.0 if engine == "cohort" else scale_func(t)
            ...
            if status[target] == 'E':
                ...
                if engine == 'cohort':
                    ...
                    if len(susceptible_neighbors) > 0:
                        p = min(1.0, target_rt / len(susceptible_neighbors))
                        infections = np.random.binomial(len(susceptible_neighbors), p)
                        np.random.shuffle(susceptible_neighbors)
                        
                        t_idx = min(int(t), max_sim_time)
                        if t_idx >= 0:
                            true_rt_numerator[t_idx] += infections
                            true_rt_denominator[t_idx] += 1
                        
                        for i in range(min(infections, len(susceptible_neighbors))):
                            neighbor = susceptible_neighbors[i]
    ...
    if return_time_series:
        if engine == 'cohort':
            return {
                ...
                'true_rt_numerator': true_rt_numerator,
                'true_rt_denominator': true_rt_denominator
            }
