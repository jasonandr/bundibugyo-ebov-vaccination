...
def plot_rt_calibration():
    ...
    # Load 100 simulations from chunk_1
    z = np.load("data_and_results/new_spaghetti_chunks/chunk_1.npz", allow_pickle=True)
    base_no_vax = z["base_no_vax"][:100]  # Take 100 trajectories
    max_sim_time = min(len(cases_inc_empirical), base_no_vax.shape[1])
    
    all_incidences = base_no_vax[:, :max_sim_time].astype(float)
    mean_sim_inc = np.mean(all_incidences, axis=0)
    simulated_rt_mean = estimate_rt_from_incidence(mean_sim_inc)
    
    # Fast Rt for spaghetti lines
    simulated_rt_spaghetti = fast_rt(all_incidences)
    # Smooth the fast Rt in time for aesthetics
    ...
    ax2.plot(df_daily['Date'][:max_sim_time], rt_array[:max_sim_time], color='#2980B9', lw=1.5, linestyle=':', label='Input $R_t$ Forcing')
    ...
    img_name = f"rt_calibration_spaghetti_{timestamp}.png"
